...
class MineDojoEnv(gym.Env):
    def __init__(
        ...
        self.action_space = mdj_spaces.MultiDiscrete(
            [3, 3, 4, 11, 11, 8, 1, 1], noop_vec=[0, 0, 0, 5, 5, 0, 0, 0]
        )  # minedojo with VPT camera view
        ...
        self._env.observation_space["rgb"] = mdj_spaces.Box(
            shape=(h, w, c), high=high, low=low
        )
    ...
    def build_obs(self, inp_obs):
        obs = {
            ...
            "compass": np.concatenate(
                [inp_obs["location_stats"]["yaw"], inp_obs["location_stats"]["pitch"]]
            ),
