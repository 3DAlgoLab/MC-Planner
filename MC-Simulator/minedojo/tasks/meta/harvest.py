...
class HarvestMeta(ExtraSpawnMetaTaskBase):
    ...
    def __init__(
        ...
        *,
        ...
        nearby_reward_weights: Optional[
            Union[int, float, Dict[str, Union[int, float]]]
        ] = None,
        nearby_target_quantities: Optional[
        ...
        if nearby_target_names is not None:
            if isinstance(nearby_reward_weights, int) or isinstance(
                nearby_reward_weights, float
            ):
                nearby_reward_weights = {
                    k: nearby_reward_weights for k in nearby_target_names
                }
        ...
        if isinstance(nearby_target_quantities, int):
            nearby_target_quantities = {
                k: nearby_target_quantities for k in nearby_target_names
            }
        elif isinstance(nearby_target_quantities, list):
            nearby_target_quantities = {
                k: nearby_target_quantities[i]
                for i, k in enumerate(nearby_target_names)
            }
        self._nearby_target_quantities = (
            nearby_target_quantities if nearby_target_names is not None else None
        )
        ...
        if isinstance(pos_target_weights, int) or isinstance(pos_target_weights, float):
            pos_and_weights = {
                pos: (pos_dirs[i], 0.0, pos_target_weights)
                for i, pos in enumerate(pos_targets)
            }
        elif isinstance(pos_target_weights, list):
            pos_and_weights = {
                pos: (pos_dirs[i], 0.0, pos_target_weights[i])
                for i, pos in enumerate(pos_targets)
            }
        ...
        if nearby_target_names is not None:
            reward_fns += [
                nearby_has_item_reward(items_and_weights=nearby_reward_weights)
            ]
