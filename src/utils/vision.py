...
class MineCLIPWrapper(MineCLIP):
    ...
    def forward(self, img, goal_embeddings):
        ...
        inp = inp.reshape(
            ...
            1,
            C,
            MineCLIPWrapper.MINECLIP_AGENT_RESOLUTION[0],
            MineCLIPWrapper.MINECLIP_AGENT_RESOLUTION[1],
        )
...
if __name__ == "__main__":
    vpt = create_backbone(
        "impala_cnn",
        "/home/caishaofei/workspace/MODEL_BASE/models/3x.model",
        "/home/caishaofei/workspace/MODEL_BASE/weights/foundation-model-3x.weights",
    )
    inp = torch.ones(2, 3, 128, 128)
    opt = vpt(inp)
    print(opt.shape)

    mineclip = create_backbone(
        "mineclip",
        weight_path="/home/caishaofei/workspace/CODE_BASE/MineCLIP-lucky/pretrained/attn.pth",
    )
    inp = torch.ones(2, 3, 128, 128)
    opt = mineclip(inp)
    print(opt.shape)
