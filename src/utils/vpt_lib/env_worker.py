...
class EnvWorker(mp.Process):
    def __init__(
        ...
        self.voxels = voxels
        self.worker_id = (
            None if recording_path is None else int(recording_path.split("_")[-1])
        )
    ...
    def run(self):
        ...
        while True:
            ...
            if command == "reset":
                ...
                self._send_message(
                    "send_obs_plus", (obs, reward, done, simplified_info)
                )
                ...
                if self.record:
                    ...
                    if (
                        ...
                        print(
                            f"> Worker {self.worker_id} completed 1 successful eps ({len(successful_eps)} buffered)"
                        )

                    if done and len(successful_eps) >= 5:
                        batches = self._batch_builder.build_and_reset_with_cond(
                            successful_eps
                        )
                        ...
                        print(
                            f"> [Json] Worker {self.worker_id} dumped 5 eps ({total_num_eps} in total)"
                        )
...
class EnvManager:
    def __init__(
        ...
        if use_lmdb and recording_path is not None:
            ...
            self.trajs_lmdb_env = lmdb.open(
                self.lmdb_trajs_path, map_size=1099511627776
            )
            self.indices_lmdb_env = lmdb.open(
                self.lmdb_indices_path, map_size=1073741824
            )
            ...
            for i in range(self.num_workers):
                ...
                curr_rec_path = (
                    None if recording_path is None else recording_path + f"_{i}"
                )
                worker = EnvWorker(
                    child_pipe,
                    self.env_generator,
    ...
    def collect_observations(self):
        ...
        for worker_idx in range(self.num_workers):
            ...
            if command == "send_obs":
                ...
                if self.worker_traj_history is not None:
                    self.worker_traj_history[worker_idx][-1].extend(
                        [reward, done, info]
                    )
    ...
    def _conditional_record(self, worker_idx):
        cum_reward = sum(
            [item[2] for item in self.worker_traj_history[worker_idx][:-1]]
        )
        accomplishments = set()
        ...
        if cum_reward > 0.0 and save_flag:
            ...
            for chunk_start in range(0, traj_len, self.lmdb_chunk_size):
                ...
                serialized_chunk = pickle.dumps(
                    self.worker_traj_history[worker_idx][chunk_start:chunk_end]
                )
            ...
            print(
                "Dumped 1 trajectory of length {} -".format(traj_len), accomplishments
            )
